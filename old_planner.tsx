...
const planSchema = z.object({
    ...
    context: z.string().min(1, "Debes seleccionar d├│nde ocurre."),
    intensity: z.string().min(1, "Selecciona la intensidad."),
    dogInfo: z.string().min(1, "Debes describir a tu perro."),
    details: z.string().min(1, "Por favor, da m├ís detalles de la situaci├│n."),
...
async function callGroq(input: GeneratePlanInput & { dogName?: string, ownerName?: string }): Promise<GeneratePlanOutput> {
  ...
  if (!apiKey) {
    throw new Error(
      "Clave de API de Groq no encontrada. Por favor, a├▒├ídela en tu archivo .env.local como NEXT_PUBLIC_GROQ_API_KEY."
    );
  }
  ...
  const prompt = `
    Eres un experto entrenador de perros y asistente virtual para "MANADA Club", especializado en el "m├⌐todo MANADA". Tu tono debe ser alentador, emp├ítico y profesional.

    El m├⌐todo MANADA se basa en 6 pilares fundamentales:
    1. Mindfulness: Estar presente y consciente del estado emocional del perro y del tuyo propio.
    2. Apego Seguro: Construir un v├¡nculo s├│lido basado en la confianza y la comunicaci├│n.
    3. Normas: Establecer reglas claras y consistentes.
    4. Autocontrol: Ense├▒ar al perro a gestionar sus impulsos.
    5. Desensibilizaci├│n: Exponer gradualmente al perro a sus miedos de manera controlada.
    6. Adaptaci├│n: Ayudar al perro a generalizar el buen comportamiento a diferentes entornos.

    Un usuario, ${input.ownerName || 'Due├▒o'}, necesita un plan de entrenamiento personalizado para su perro, ${input.dogName || 'su perro'}.
    
    **Aqu├¡ est├í la informaci├│n estructurada que proporcion├│:**
    ...
    - **Comorbilidades del Gu├¡a:** ${input.comorbidities?.join(', ') || 'Ninguna especificada'}
    - **Situaci├│n Detallada (El Por Qu├⌐):** "${input.details}"

    **Tu Tarea:**
    Utilizando toda la informaci├│n proporcionada, crea un plan de entrenamiento paso a paso estructurado, pr├íctico y altamente personalizado siguiendo el m├⌐todo MANADA.

    1. An├ílisis: Analiza brevemente la situaci├│n desde la perspectiva del m├⌐todo MANADA. Conecta la descripci├│n detallada del usuario con los 6 pilares. ┬┐Qu├⌐ pilares son m├ís relevantes aqu├¡ y por qu├⌐?
    2. ├üreas de Enfoque: Identifica los 2 o 3 pilares principales del m├⌐todo MANADA en los que centrarse. Estos deben abordar directamente el 'Problema Principal' y el 'Contexto'.
    3. Pasos del Plan: Proporciona un plan concreto, paso a paso. Cada paso debe ser un ejercicio pr├íctico con un t├¡tulo y una duraci├│n/frecuencia recomendada. Las instrucciones deben ser muy claras y f├íciles de seguir para un due├▒o novato. Adapta los ejercicios a la 'Informaci├│n del Perro' (ej: sesiones m├ís cortas para un cachorro, enfoques diferentes para un perro con mucha energ├¡a).
    4. Consejo Pro: Ofrece un "consejo pro" final y alentador que se alinee con la filosof├¡a MANADA y sea directamente relevante para el problema espec├¡fico del usuario.
  ...
  if (!content) {
    throw new Error("El modelo de IA no devolvi├│ un plan v├ílido.");
  }
    ...
    return (
        <div className="mt-6 space-y-6">
            <Alert className="bg-primary/5 border-primary/20">
                ...
                <AlertTitle className="text-primary font-bold">An├ílisis de la Situaci├│n</AlertTitle>
            ...
            <Card className="border-dashed bg-muted/30">
                <CardHeader className="pb-3">
                    <CardTitle className="text-sm font-bold flex items-center gap-2">
                        ...
                        ┬┐Te ha sido ├║til este plan?
                    </CardTitle>
                    <CardDescription className="text-xs">
                        Tu opini├│n nos ayuda a mejorar las recomendaciones de la IA.
                    </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                    {!submitted ? (
                        ...
                        <div className="text-center py-4 space-y-2">
                            ...
                            <p className="text-sm font-bold text-foreground">┬íGracias por tu feedback!</p>
        ...
        <div className="mt-6 space-y-6 animate-pulse">
            <div className="flex items-center gap-3 p-4 rounded-lg bg-primary/5 border border-primary/10">
                ...
                <div className="space-y-2 flex-1">
                    <p className="text-sm font-medium text-primary">El Asistente MANADA est├í analizando tu caso...</p>
...
export function PlannerCard({ className }: { className?: string }) {
  ...
  const onSubmit = async (values: z.infer<typeof planSchema>) => {
    ...
    try {
      ...
      toast({
        title: "┬íPlan Generado!",
        description: `Tu ruta para ${selectedPet.name} est├í lista.`,
      });
    ...
    <Card className={cn("border-white/5 bg-black/40 backdrop-blur-xl rounded-[2.5rem]", className)}>
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-xl font-black uppercase tracking-tighter text-white">
             ...
             Diagn├│stico IA: {selectedPet?.name || "Binomio"}
          </CardTitle>
        ...
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)}>
                <CardContent className="space-y-6">
                    <div className="space-y-4">
                        <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">┬┐Cu├íl es el foco hoy?</Label>
                        <FormField
                                ...
                                <FormItem>
                                    <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-2">
                                        {[
                                            ...
                                            { id: 'Destructive', label: 'Destrucci├│n', icon: '/focus/destruction.png' },
                                            { id: 'Aggression', label: 'Agresividad', icon: '/focus/aggression.png' },
                                            { id: 'Guarding', label: 'Protecci├│n Recursos', icon: '/focus/guarding.png' },
                                            ...
                                            { id: 'Attention', label: 'Busca Atenci├│n', icon: '/focus/attention.png' },
                                        ].map((opt) => (
                                            <div 
                                                key={opt.id}
                    ...
                    <div className="space-y-4 pt-2">
                        <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Escenario de la situaci├│n</Label>
                        <FormField
                            control={form.control}
                            name="context"
                            render={({ field }) => (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-2">
                        ...
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    <FormLabel className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Perfil del Perro (Edad, Raza, Socializaci├│n)</FormLabel>
                                    <FormControl>
                                        <Input 
                                            placeholder="Ej: Golden de 2 a├▒os, muy activo." 
                    ...
                    <div className="space-y-4 pt-2">
                        <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Comorbilidades del Gu├¡a (Opcional)</Label>
                        <FormField
                            control={form.control}
                            name="comorbidities"
                            render={({ field }) => (
                                <FormItem>
                                    <div className="flex flex-wrap gap-2">
                                        {['TEPT', 'Ansiedad', 'Fobias', 'Depresi├│n', 'TEA'].map((item) => (
                                            <Badge
                                                key={item}
                    ...
                    <div className="space-y-4 pt-2">
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    <FormLabel className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Situaci├│n Detallada (El Por Qu├⌐)</FormLabel>
                                    <FormControl>
                                        <Textarea
                                            placeholder="Cu├⌐ntanos exactamente qu├⌐ sucede..."
